...
export default function Programmers() {
  ...
  const [activeProgrammerId, setActiveProgrammerId] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    const getProgrammer = programmers.filter((item) => item.id === activeProgrammerId);

    return getProgrammer[0].name;
  };

  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: activeProgrammerId !== null ? "gold" : "royalblue", // 🤔 color turns to gold, when celebrating
  };
